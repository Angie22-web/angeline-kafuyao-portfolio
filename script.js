...
function render(){const matches=projects.filter(p=>filter==='All'||p[1]===filter);grid.innerHTML=matches.slice(0,limit).map((p,i)=>{const v=visual[p[1]]||[p[1],"✦"];return `<article class="project project-${p[1].toLowerCase().replaceAll(' ','-')}"><div class="project-art"><span class="art-number">${String(i+1).padStart(2,'0')}</span><span class="art-icon" aria-hidden="true">${v[1]}</span><span class="art-label">${v[0]}</span></div><div class="project-body"><div class="meta"><span>${p[1]}</span><span>${p[2]}</span></div><h3>${p[0]}</h3><a href="${p[3]}" target="_blank" rel="noreferrer">View published work <span aria-hidden="true">↗</span></a></div></article>`}).join('');count.textContent=matches.length;load.hidden=limit>=matches.length}
